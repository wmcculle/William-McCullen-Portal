...
namespace
{
	...
	{
		if (!Math::NearlyEqual(expected, actual, 0.3f))
		{
			   ...
			   mLastTimestamp >= mEvents[mCurrentPlaybackIndex + 1].mTimestamp)
		{
			...
			if (mEnableValidation)
			{
				...
				if (event.mBluePortal.mUpdated)
				{
					...
					if (event.mBluePortal.mExists)
					{
						if (!mBluePortal)
						{
							SDL_LogWarn(
								0,
								"Blue portal mismatch.\nExpected: Exists\nActual:   Does not exist");
							SDL_assert(
								false &&
								"InputReplay validation failed, check output log for details or break for debugger");
						}
						...
						{
							ValidateVector("Blue portal position", event.mBluePortal.mPosition,
										   mBluePortal->GetPosition());
							ValidateQuat("Blue portal quat", event.mBluePortal.mQuat,
										 mBluePortal->GetQuat());
							...
							CollisionComponent* cc =
								mBluePortal->GetComponent<CollisionComponent>();
							if (cc)
							{
								width = cc->GetWidth();
								height = cc->GetHeight();
							...
							ValidateFloat("Blue portal width", event.mBluePortal.mWidth, width);
							ValidateFloat("Blue portal width", event.mBluePortal.mHeight, height);
							ValidateFloat("Blue portal width", event.mBluePortal.mDepth, depth);
						}
					}
				...
				if (event.mOrangePortal.mUpdated)
				{
					...
					if (event.mOrangePortal.mExists)
					{
						if (!mOrangePortal)
						{
							SDL_LogWarn(
						...
						{
							ValidateVector("Orange portal position", event.mOrangePortal.mPosition,
										   mOrangePortal->GetPosition());
							ValidateQuat("Orange portal quat", event.mOrangePortal.mQuat,
							...
							float depth = 0.0f;
							CollisionComponent* cc =
								mOrangePortal->GetComponent<CollisionComponent>();
							if (cc)
							{
								width = cc->GetWidth();
								height = cc->GetHeight();
							...
							ValidateFloat("Orange portal width", event.mOrangePortal.mWidth, width);
							ValidateFloat("Orange portal width", event.mOrangePortal.mHeight,
										  height);
							ValidateFloat("Orange portal width", event.mOrangePortal.mDepth, depth);
						}
					}
...
{
	// Reset initial player position/camera
	mGame->GetPlayer()->SetPosition(mGame->GetPlayer()->GetInitialPos());
	mGame->GetPlayer()->SetRotation(0.0f);
	mGame->GetPlayer()->GetCameraComponent()->ResetPitchAngle();
	mGame->GetPlayer()->GetPlayerMove()->Reset();
}
