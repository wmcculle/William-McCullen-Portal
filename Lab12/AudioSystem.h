...
private:
	...
	struct HandleInfo
	{
		...
		class Actor* mActor = nullptr; // to track the actor, if any
		bool mStopOnActorRemove =
			true; // to note whether the sound should stop if the actor is removed
	};
	...
	float mVolumeInnerRadius = 25.0f;
	float mVolumeOuterRadius = 600.0f;
};
